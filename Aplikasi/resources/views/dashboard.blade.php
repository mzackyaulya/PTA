                ...
                @forelse($banners->take(3) as $key => $banner)
                    <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                        <div class="position-relative banner-wrapper">
                            <button class="btn-close position-absolute bg-white p-2"
                                style="top:10px; right:10px; z-index:10;" onclick="hideBanner()">
                            </button>
                            <img src="{{ asset('storage/' . $banner->image_path) }}" class="d-block w-100 rounded"
                                style="height:400px; object-fit:cover;" alt="Banner">
                            @if (auth()->user()->role === 'admin')
                                <div class="position-absolute top-50 start-50 translate-middle d-none edit-btn text-center">
                                    <a href="{{ route('banners.edit', $banner->id) }}"
                                        class="btn btn-transparant fw-bold px-4 py-2 fs-5 text-white">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body d-flex justify-content-between align-items-center">
                ...
                <form action="{{ route('announcements.index') }}" method="GET" class="d-flex" style="max-width: 350px;">
                    <input type="text" name="search" class="form-control form-control-lg rounded-pill me-2"
                        ...
                        <div class="card-body p-0 d-flex flex-column">
                            ...
                            @if ($a->image_path)
                                <div style="flex-grow:1; max-height:250px; overflow:hidden;">
                                    <img src="{{ asset('storage/' . $a->image_path) }}" class="w-100"
                                        style="object-fit:cover; height:100%;" alt="Pengumuman">
                                </div>
                            @endif
                            ...
                            <div class="border-top p-3 mt-auto">
                                <a href="#" class="d-inline-flex align-items-center text-primary text-decoration-none"
                                    data-bs-toggle="offcanvas" data-bs-target="#announcementDetail{{ $a->id }}">
